...
public class User extends javax.swing.JFrame {
    ...
    private void usersMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_usersMouseClicked
        ...
        try {
            ...
            if (getRes().next()) {
                String Id = getRes().getString("ID");
                id.setText(Id);

                String Name = getRes().getString("NAME");
                name.setText(Name);

                String Day = getRes().getString("DOB").split("-")[0];
                day.setSelectedItem(Day);
                String Month = getRes().getString("DOB").split("-")[1];
                month.setSelectedItem(Month);
                String Year = getRes().getString("DOB").split("-")[2];
                year.setSelectedItem(Year);

                String Address = getRes().getString("ADDRESS");
                address.setText(Address);

                String Phone = getRes().getString("PHONE");
                phone.setText(Phone);

                String Salary = getRes().getString("SALARY");
                salary.setText(Salary);

                String Password = getRes().getString("PASSWORD");
    ...
    private void refresh() {
        ...
        try {
            setPre(getCon().prepareStatement(sql));
            setRes(getPre().executeQuery());
            users.setModel(DbUtils.resultSetToTableModel(getRes()));
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null, e.getMessage(), "Error", 2);
        }
    }
